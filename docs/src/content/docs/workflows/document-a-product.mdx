...
**Scaffold.** README first: its structure section lists every document expected,
grouped by how the user meets the feature, never by package or module, and the
coverage table marks each `not started`. Then the glossary, then `goal.md`, then
`AGENTS.md` and `CLAUDE.md`. Commit: `docs: add README, glossary, goal`.
...
Result columns, and states in `verification/README.md` what the pass did and did not
cover. No document is marked `verified` on an automated pass alone. A failed row is
...
call`). A summary table at the top is sorted by severity.
