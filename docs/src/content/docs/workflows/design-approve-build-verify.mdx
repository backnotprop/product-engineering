...
Verify (the whole path, states, viewports, zoom, focus, console, at most two visual
rounds). What comes back is code in your stack plus a report: what changed, what was
preserved, what remains, and the record built against with each stated deviation.
...
states, copy, spacing, and behavior, in that order, at every viewport the artifact
demonstrates.
...
Scope names the target, the commit, and how the product is reached, and checks that
Playwright has a browser; when it does not, the skill offers
`npx playwright install chromium` in one line and continues either way.
