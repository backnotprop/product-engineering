...
1. **Monogram + Meaning** — the brand initial combined with a metaphor (`K` + kite /
   frame / direction; `S` + sound wave). Never a plain letter icon: use negative
   space, cuts, folds, or geometry.
2. **Product Action** — the product's main verb as a symbol: build → frame, scaffold,
   block; protect → shield, boundary, watch mark; convert → switch, arrow; speak →
   waveform, pulse; automate → loop, handoff, path. Abstract and premium, not literal.
3. **Metaphor Fusion** — two meaningful ideas in one reduced mark (shield + mountain,
   moon + waveform, cursor + lightning). The fusion must stay subtle and readable.
4. **Negative Space** — empty space that creates intelligence: hidden arrow, protected
   center, cutout initial, internal path, folded corner. Negative space must be crisp.
5. **Construction Geometry** — a mark from a visible system: circles, diagonal cuts,
   grids, modular blocks, orbital paths, measured linework. One asset may show the
   construction logic itself.

## Board and panel composition

A brand board (or any multi-asset set) is a curated sequence, not a grid of equals:
...
Short and specific: "What will you build today?" · "Nothing random." · "Your network.
Our watch." · "On guard." · "Clarity builds confidence." Never generic corporate
slogans, long marketing copy, buzzword soup, or fake inspirational fluff. A tagline
panel is quiet: one short line, large type, calm ground.
