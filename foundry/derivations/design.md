...
date/by, exact approved state, decisions on the way, out of scope). Newer approvals
supersede; superseded folders are renamed with their date, never deleted; records are
never edited except to mark them superseded. `.product/` is created on first use with
